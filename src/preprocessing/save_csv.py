...
def process_csv(data_dir,N):
	...
	for current_class in range(total_classes):
		label_indexes = np.random.choice(np.where(labels_list==current_class)[0],int(N/total_classes),replace = True)
	...
	df = pd.DataFrame(triplets_list, columns=["anchor_image","positive_image","neg"])
	# df.to_csv(os.path.join(data_dir,"train",str("train") + "_"+str(N)+".csv"), encoding="utf-8", index=False)
